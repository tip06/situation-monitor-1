<script lang="ts">
	...
	import { settings, language } from '$lib/stores';
...
<Modal {open} title={t($language, 'settings.title')} {onClose}>
	<div class="settings-sections">
		<section class="settings-section">
			<h3 class="section-title">{t($language, 'settings.enabledPanels')}</h3>
			<p class="section-desc">{t($language, 'settings.toggleDescription')}</p>
			...
			<div class="panels-grid">
				{#each Object.entries(PANELS) as [id, config]}
					...
					<label class="panel-toggle" class:enabled={isEnabled}>
						...
						/>
						<span class="panel-name">{t($language, config.nameKey)}</span>
		...
		<section class="settings-section">
			<h3 class="section-title">{t($language, 'settings.dashboard')}</h3>
			{#if onReconfigure}
				<button class="reconfigure-btn" onclick={onReconfigure}>
					{t($language, 'settings.reconfigure')}
				</button>
				<p class="btn-hint">{t($language, 'settings.reconfigureHint')}</p>
			{/if}
			<button class="reset-btn" onclick={handleResetPanels}>{t($language, 'settings.resetAll')}</button>
		</section>
	</div>
</Modal>
